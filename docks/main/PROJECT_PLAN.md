...
- Механизм TFIM-потолка не проверен напрямую. Теория (мешение секторов
  чётности диссипатором) согласуется с данными, но прямой тест — диссипатор,
  коммутирующий с чётностью: при нём деградация должна исчезнуть.
